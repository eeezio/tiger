...
static void Arg_setCollectInfoShow(int flag)
{
  show_collect_info = flag;
  return;
}
...
static struct Arg_t allArgs[] = {
    ...
    {"gcLog",
     "<n>",
     "set whether to display GC information",
     ARGTYPE_INT,
     Arg_setCollectInfoShow}};
...
void CommandLine_doarg(int argc, char **argv)
{
  ...
  for (index = 1; index < argc;)
  {
    ...
    // {
    if (allArgs[solt_count].action)
    {
      inputName = argv[index++];
      if (strcmp(inputName + 1, allArgs[solt_count].name) != 0)
      {
        continue;
      }
      switch (allArgs[solt_count].argtype)
      {
      case ARGTYPE_BOOL:
      {
        int b;
        ...
        if (index >= argc)
          errorNoArg(allArgs[solt_count].name,
                     allArgs[solt_count].arg);
        ...
        if (strcmp(arg, "true") == 0)
          ...
          errorWrongArg(allArgs[solt_count].name,
                        allArgs[solt_count].arg,
                        arg);
        allArgs[solt_count].action(b);
        break;
      }
      case ARGTYPE_INT:
      {
        ...
        arg = argv[index++];
        result = atoi(arg);
        allArgs[solt_count].action(result);
        break;
      }
      case ARGTYPE_STRING:
      {
        ...
        arg = argv[index++];
        allArgs[solt_count].action(arg);
        break;
      }
      case ARGTYPE_EMPTY:
      {
        allArgs[solt_count].action();
        break;
      }
      default:
      {
