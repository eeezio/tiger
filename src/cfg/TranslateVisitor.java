...
public class TranslateVisitor implements codegen.C.Visitor {
    ...
    public void visit(codegen.C.Ast.Exp.Add e) {
        String dst = genVar();
        e.left.accept(this);
        Operand.T left = operand;
        e.right.accept(this);
        emit(new Stm.Add(dst, null, left, operand));
    }
    ...
    public void visit(codegen.C.Ast.Exp.And e) {
        ...
        Operand.T left = operand;
        e.right.accept(this);
        emit(new And(dst, null, left, operand));
    }
    ...
    public void visit(codegen.C.Ast.Exp.ArraySelect e) {
        String dst = genVar();
        e.index.accept(this);
        Operand.T index = operand;
        //array应该就是Id
        e.array.accept(this);
        Operand.IntArray array = new Operand.IntArray(operand.toString(), index);
        emit(new Move(dst, null, array));
        operand = new Var(dst);
    }
    ...
    //从这里不必太过关心如何获取length，因为这个可以在代码生成的时候从obj头找到。
    @Override
    public void visit(codegen.C.Ast.Exp.Length e) {
        //这里应该也是个var id。
        e.array.accept(this);
        emit(new Length(e.assign, operand));
        operand = new Var(e.assign);
    }
    ...
    public void visit(codegen.C.Ast.Exp.NewIntArray e) {
        ...
        if (e.name == null) {
            ...
            emit(new NewIntArray(dst, operand));
            this.operand = new Var(dst);
        } else {
            emit(new NewIntArray(e.name, operand));
    ...
    public void visit(codegen.C.Ast.Stm.AssignArray s) {
        ...
        Operand.T index = operand;
        s.exp.accept(this);
        Operand.IntArray array = new Operand.IntArray(s.id, index);
        emit(new Move(array.toString(), null, operand));
        operand = array;
    }
    ...
    public void visit(codegen.C.Ast.Stm.While s) {
        Label lb = new Label();
        util.Label tl = new Label();
        Label fl = new Label();
        emit(lb);
        s.condition.accept(this);
        emit(new If(operand, tl, fl));
        emit(tl);
        s.body.accept(this);
        emit(new Goto(lb));
        emit(fl);
    ...
    public void visit(codegen.C.Ast.Type.IntArray t) {
        this.type = new Type.IntArrayType();
    }
